...
const selectedPokemon = pokemon.filter(p => p.type === "fire" && p.hp > 40).slice(0, 3);
...
if (starterPokemon) {
  let evolvedPokemon = pokemon.find(p => {
    return p.number === starterPokemon.number + 1;
  });

  
  if (evolvedPokemon) {
    let starterIndex = game.party.indexOf(starterPokemon);

    game.party[starterIndex] = evolvedPokemon;
  }
}
console.log(game.party);
